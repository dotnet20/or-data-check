    ...
    public class ExcelService : IExcelService
    {
        ...
        public DataTable LoadData(string filePath, int maxRows = 10, int maxCols = 5)
        {
            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                {
                    string colName = ws.Cells[1, c].Text; //to raczej bardziej przeszkadza niz pomaga, lepiej, żeby były tylko "Column n"
                    dt.Columns.Add(string.IsNullOrWhiteSpace(colName) ? $"Kolumna {c}" : colName); // Kolumna ? tak po polsku ? wszystko powinno być po angielsku
                }
